	...
	function doBuild(buildGroup) {
		...
		try{
			...
			minion.require(classes, function() {
				...
				for(i = loadedClasses.length-1; i >= 0; i --) {
				...
				for(i = 0; i < loadedClasses.length; i ++) {
					...
					if(jshint_check) {
						...
						if(!jshint(getFileContents(file), jshint_opts)) {
							...
							jshint_errors = jshint_errors.concat(jshint.errors);
						}
					}
				...
				if(jshint_errors.length === 0) {
					...
					cb(err("JSHINT Error."), jshint_errors);
				}
				return;
			});
		}
...
function fileChangedFn (files, cb) {
...
function watchFiles (files, cb) {
	...
	for(var i = 0; i < files.length; i ++) {
		var file = files[i];
		fs.watchFile(file, {persistent: true, interval: 50}, fileChangedFn(files, cb));
	}
}
...
function uglify (file) {
	...
	code = ug.ast_mangle(code);
	code = ug.ast_squeeze(code);
	code = ug.gen_code(code);
