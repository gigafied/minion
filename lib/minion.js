...
minion.build = function (opts, cb, _doLog) {
	...
	if(config) {
		...
		console.log(basePath, config.class_path);
	}
	...
	// Otherwise, build the config obj ourselves	
		...
		config = {
			build_groups: [{
				...
				"output" : path.normalize(opts.class_path + "/" + opts.output),
	...
	function doBuild(buildGroup) {
		...
		try{
			...
			minion.require(classes, function() {
				...
				fs.writeFile(buildGroup.output, output);	

				provides.push({
					file : (buildGroup.output_path || config.output_path) + path.basename(buildGroup.output),
				...
				log("----------------------------------------------------------------------------------------");
				log("");
				log("minion.configure({paths: " + JSON.stringify(provides, null, 4) + "});");
				log("");
				log("----------------------------------------------------------------------------------------");
				log("");
